...
export default function Navbar() {
  ...
  return <header className="sticky top-0 z-50 border-b border-white/[0.08] bg-background/75 backdrop-blur-2xl">
    <nav className="page-container flex h-[72px] items-center justify-between" aria-label="Navegação principal">
      <motion.button initial={{ opacity: 0, x: -18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .5 }} onClick={() => goTo('home')} className="flex items-center gap-3" aria-label="Ir para o início">
      ...
      <motion.div initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .5, delay: .1 }} className="hidden items-center gap-1 md:flex">
        {links.map(link => <button key={link.id} onClick={() => goTo(link.id)} className={`relative rounded-lg px-4 py-2 text-sm font-medium transition ${active === link.id ? 'text-primary' : 'text-muted-foreground hover:text-white'}`}>{active === link.id ? <motion.span layoutId="active-navigation" className="absolute inset-0 -z-10 rounded-lg bg-primary/10 ring-1 ring-primary/15" transition={{ type: 'spring', stiffness: 350, damping: 30 }} /> : null}{link.name}</button>)}
