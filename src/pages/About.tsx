...
export default function About() {
  return <div className="section-space page-container">
    ...
    <div className="mt-8 rounded-2xl border border-white/[0.08] bg-secondary/35 p-7"><p className="mb-5 text-sm font-semibold uppercase tracking-[.2em] text-muted-foreground">Tecnologias e ferramentas</p><div className="flex flex-wrap gap-2">{skills.map(skill => <span key={skill} className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-white/80 transition hover:border-primary/35 hover:text-primary">{skill}</span>)}</div></div>
  </div>;
}
